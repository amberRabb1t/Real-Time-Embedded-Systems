...
static void *everyInterval (void *args) {
    ...
    while (!intDat->finish) {
        ...
        /* Calculation of Maximum Pearson Correlation Coefficient: for each instrument, what is computed is the coefficient of its latest "average price" vector with all past (and current, if applicable) "average price" vectors
           of every other instrument (and itself), in order. The largest coefficient found is then saved as this interval's coefficient: its value, how far back in time it was found, and the instrument it was found with. */
        for (i = 0; i < SUBSCRIPTIONS; ++i) {
            bestIndicator = -1;
            maxPearson.coeff = 0;
