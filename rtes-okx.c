...
static int callback_okx(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len) {
    switch (reason) {
        ...
        case LWS_CALLBACK_CLIENT_RECEIVE: {
			...
			// Looking at the OKX WebSocket API documentation, trade data messages have no "type" attribute; this is used to distinguish between the different kinds of messages
            if (type != NULL) {
				...
				// An "unsubscribe" message means an instrument has (somehow) been unsubscribed from (never happened in practice)
					...
					// Decrement the subscription counter and copy the instrument-ID of the unsubscribed-from instrument to "subList", which is the list containing the instruments to subscribe to
                ...
                if (data != NULL) {
					...
					// Iterate through the (const) "subscriptions" array until the index that the incoming instrument-ID corresponds to is found
                    for (int i = 0; i < SUBSCRIPTIONS; ++i) {
                        if (strcmp(instId_value, proDat->subscriptions[i]) == 0) {
                            ...
                            currentTrade.count = strtod((const char *)json_string_value(cnt_obj), NULL); // one message may aggregate multiple trades; this is how many were aggregated
                            currentTrade.price = strtod((const char *)json_string_value(px_obj), NULL);
                            currentTrade.size = strtod((const char *)json_string_value(sz_obj), NULL); // if many trades were aggregated, this is the TOTAL size of ALL of them combined
		...
		// If the connection is randomly closed, attempt reconnection indefinitely
        case LWS_CALLBACK_CLIENT_CLOSED: {
            printf("WebSocket connection closed\n");
            producerData *proDat = lws_context_user(lws_get_context(wsi));
            if (!proDat->interrupted) {
                lws_retry_sul_schedule_retry_wsi(wsi, &proDat->ccDat.sul, connect_client, &proDat->ccDat.retry_count); // for more information, see "connect_client" function
            }
            return 0;
        }
		
		// If the connection is closed due to error, attempt reconnection indefinitely
...
int main() {
/*  The program is designed to run until manually terminated. As such, we want to handle SIGINT and SIGTERM ourselves, and perform cleanup before exiting. This is achieved via signal masking.
    "Harsher" signals like SIGQUIT are left unhandled as there should be a way to end the program on the spot, if desired. */
...
static void *consumer (void *args) {
    ...
    while (!conDat->consumerFlag) {
		...
		// Add the trade's price/size to the interval's data [for this instrument]
...
static void *everyInterval (void *args) {
    ...
    while (!intDat->finish) {
        ...
        while (!intDat->finish && pthread_cond_timedwait(&intDat->finishUp, &intDat->DB->mut, &ts) != ETIMEDOUT); // unless the timer runs out or program shutdown is initiated, wait
        ts.tv_sec += INTERVAL; // upon non-spurious wakeup, *immediately* set the time struct one INTERVAL into the future, so that no "drift" occurs due to processing delays
        ...
        for (i = 0; i < SUBSCRIPTIONS; ++i) {
			...
			/* Each instrument has a <DATAPOINTS>-long "average price" vector (updated every interval) stored for Pearson correlation coefficient calculations.
			   For the first <DATAPOINTS> intervals (minutes), this vector is simply filled in order. */
            if (minuteCount < DATAPOINTS) {
                ...
                fprintf(intDat->DB->files[1][i], "%lf,%lf,%llu\n", priceAvg[i][minuteCount], sizeTotal, unixTimeInMs()); // no other thread ever accesses these files so no need to protect access with a mutex
            }
			/* After the first <DATAPOINTS> minutes, every element of the vector is first shifted one spot towards the "head", while the "head" is discarded. The newest entry is added to the "tail".
			   This is necessary to make sure the vector is always aligned oldest-to-newest, which is required for some edge cases (relating to potential NaN entries, which can happen if e.g. no data comes for an entire INTERVAL).*/
            else {
                ...
                fseek(intDat->DB->files[1][i], 0, SEEK_END); // make sure to write at the end of the file
                fprintf(intDat->DB->files[1][i], "%lf,%lf,%llu\n", priceAvg[i][DATAPOINTS-1], sizeTotal, unixTimeInMs()); // no other thread ever accesses these files so no need to protect access with a mutex
            }
        }
        ++minuteCount;

        /* Calculation of Maximum Pearson Correlation Coefficient: for each instrument, what is computed is the coefficient of its latest "average price" vector with all past "average price" vectors
		   of every other instrument, in order. The largest coefficient found is the one saved for this interval: its value, how far back in time it was found, and the instrument it was found with. */
        for (i = 0; i < SUBSCRIPTIONS; ++i) {
            bestIndicator = -1;
            maxPearson.coeff = 0;
